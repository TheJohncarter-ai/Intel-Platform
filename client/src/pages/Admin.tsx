...
export default function Admin() {
  ...
  const tabs: { key: Tab; label: string; shortLabel: string; icon: React.ReactNode }[] = [
    { key: "overview",  label: "Overview",         shortLabel: "Overview",  icon: <BarChart3 size={12} /> },
    { key: "requests",  label: "Access Requests",  shortLabel: "Requests", icon: <Users size={12} /> },
    { key: "whitelist", label: "Whitelist",         shortLabel: "Whitelist", icon: <Shield size={12} /> },
    { key: "audit",     label: "Audit Log",         shortLabel: "Audit",    icon: <Activity size={12} /> },
  ];
    ...
    <div className="min-h-screen" style={{ background: "#0a0c18" }}>
      ...
      <div style={{ borderBottom: "1px solid #151f38", background: "rgba(6,9,20,0.92)", backdropFilter: "blur(16px)" }}>
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex items-center gap-2 sm:gap-4">
          ...
          <div className="w-[3px] h-4 sm:h-5 rounded-sm bg-[#d4a843]" style={{ boxShadow: "0 0 10px rgba(212,168,67,0.5)" }} />
          <Shield className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#d4a843]" />
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 pt-4 sm:pt-5">
        <div className="flex gap-0.5 sm:gap-1 overflow-x-auto pb-0.5 -mx-3 px-3 sm:mx-0 sm:px-0" style={{ borderBottom: "1px solid #151f38" }}>
          {tabs.map(tab => (
            <button
              key={tab.key}
              onClick={() => setActiveTab(tab.key)}
              className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-4 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-bold tracking-[0.1em] sm:tracking-[0.12em] uppercase transition-all whitespace-nowrap shrink-0"
              style={{
  ...
  return (
    <div className="space-y-5 sm:space-y-6 pb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
        {cards.map(card => (
          <div key={card.label} className="p-3 sm:p-4 rounded-lg transition-colors hover:border-[#1a3a6a]"
            style={{ background: "#060914", border: "1px solid #151f38" }}>
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <span style={{ color: card.color }}>{card.icon}</span>
              <span className="text-[#4a6080] text-[8px] sm:text-[9px] font-extrabold tracking-[0.12em] sm:tracking-[0.15em] uppercase"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                {card.label}
              </span>
            </div>
            <div className="text-xl sm:text-2xl font-bold" style={{ fontFamily: "'Syne', sans-serif", color: card.color }}>
              {card.value}
            </div>
          </div>
        ))}
...
function InviteSection() {
  ...
  return (
    <div className="p-3 sm:p-5 rounded-lg" style={{ background: "#060914", border: "1px solid #151f38" }}>
      <div className="flex items-center gap-2 mb-3 sm:mb-4">
        <Send size={14} className="text-[#34d399]" />
        <span className="text-[#34d399] text-[9px] sm:text-[10px] font-extrabold tracking-[0.18em] uppercase"
          style={{ fontFamily: "'JetBrains Mono', monospace" }}>
          Invite User
        </span>
      </div>
      <p className="text-[#4a6080] text-[10px] sm:text-xs mb-3 sm:mb-4" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:items-start">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2.5 rounded text-xs sm:text-sm text-[#c8d8f0] placeholder-[#2a3a54] bg-[#0a0c18] border border-[#151f38] outline-none focus:border-[#1a3a6a] transition-colors"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
                onKeyDown={(e) => {
            ...
            <button
              ...
              className="px-2.5 sm:px-3 py-2.5 rounded text-[9px] sm:text-[10px] font-bold tracking-wider uppercase transition-all shrink-0"
              style={{
                fontFamily: "'JetBrains Mono', monospace",
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded text-xs font-bold tracking-wider uppercase disabled:opacity-40 transition-all shrink-0 w-full sm:w-auto"
          style={{
            fontFamily: "'JetBrains Mono', monospace",
    ...
    <div className="space-y-5 sm:space-y-6 pb-8">
      ...
      {pending.length === 0 ? (
        ...
        <div className="space-y-2">
          {pending.map(req => (
            <div key={req.id} className="p-3 sm:p-4 rounded-lg transition-colors hover:border-[#1a3a6a]"
              style={{ background: "#060914", border: "1px solid #151f38" }}>
              {/* Stacks on mobile */}
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-[#c8d8f0] text-xs sm:text-sm font-semibold" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {req.name || "Unknown"}
                    </span>
                    <span className="text-[#4a6080] text-[10px] sm:text-xs truncate" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {req.email}
                    </span>
                  </div>
                  {req.reason && (
                    <p className="text-[#4a6080] text-[10px] sm:text-xs mt-1" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {req.reason}
                    </p>
                  )}
                  <span className="text-[#2a3a54] text-[9px] sm:text-[10px] mt-1.5 block" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 rounded text-[10px] font-bold tracking-wider uppercase transition-all hover:bg-[rgba(74,222,128,0.15)] active:bg-[rgba(74,222,128,0.2)]"
                    style={{
                      fontFamily: "'JetBrains Mono', monospace",
                      background: "#0d1f0d",
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 rounded text-[10px] font-bold tracking-wider uppercase transition-all hover:bg-[rgba(248,113,113,0.15)] active:bg-[rgba(248,113,113,0.2)]"
                    style={{
                      fontFamily: "'JetBrains Mono', monospace",
                      background: "#1f0d0d",
      ...
      {resolved.length > 0 && (
        <>
          ...
          <div className="space-y-2">
            {resolved.map(req => (
              <div key={req.id} className="p-3 rounded-lg opacity-70"
                style={{ background: "#060914", border: "1px solid #151f38" }}>
                <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
                  <span className={`text-[9px] sm:text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded self-start ${
                    req.status === "approved"
                  ...
                  <span className="text-[#c8d8f0] text-xs sm:text-sm truncate" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                    {req.email}
                  </span>
    ...
    <div className="space-y-5 sm:space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="email@example.com"
            className="w-full pl-8 pr-3 py-2.5 rounded text-xs sm:text-sm text-[#c8d8f0] placeholder-[#2a3a54] bg-[#060914] border border-[#151f38] outline-none focus:border-[#1a3a6a] transition-colors"
            style={{ fontFamily: "'JetBrains Mono', monospace" }}
            onKeyDown={(e) => {
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded text-xs font-bold tracking-wider uppercase disabled:opacity-40 transition-all w-full sm:w-auto"
      ...
      {!entries?.length ? (
        ...
        <div className="space-y-1">
          {entries.map(entry => (
            <div key={entry.id} className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg group transition-colors hover:border-[#1a3a6a]"
              style={{ background: "#060914", border: "1px solid #151f38" }}>
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                <div className="w-1.5 h-1.5 rounded-full bg-[#4ade80] shrink-0" style={{ boxShadow: "0 0 6px rgba(74,222,128,0.5)" }} />
                <span className="text-[#c8d8f0] text-xs sm:text-sm truncate" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  {entry.email}
                </span>
                <span className="text-[#2a3a54] text-[9px] sm:text-[10px] hidden sm:inline shrink-0" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
              ...
              <button
                ...
                className="sm:opacity-0 sm:group-hover:opacity-100 text-[#f87171] hover:text-[#fca5a5] active:text-[#fca5a5] transition-all p-1.5"
              >
                <UserMinus size={14} />
              </button>
            </div>
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 -mx-3 px-3 sm:mx-0 sm:px-0">
        <span className="text-[#4a6080] text-[9px] sm:text-[10px] tracking-[0.18em] uppercase shrink-0"
        ...
        <>
          <div className="space-y-1">
            {data?.entries.map(entry => {
              ...
              return (
                <div key={entry.id} className="px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg transition-colors hover:border-[#1a3a6a]"
                  style={{ background: "#060914", border: "1px solid #151f38" }}>
                  {/* Desktop: single row */}
                  ...
                  <div className="sm:hidden">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[#c8d8f0] text-[10px] truncate" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                        {entry.actorName || entry.actorEmail}
                      </span>
                    </div>
                    {entry.details && (
                      <p className="text-[#4a6080] text-[10px] truncate mb-1" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                        {entry.details}
                      </p>
                    )}
                    <span className="text-[#2a3a54] text-[9px]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {new Date(entry.createdAt).toLocaleString()}
                    </span>
                  </div>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-4 pt-4">
              <button
                ...
                disabled={page === 1}
                className="text-[#4a6080] hover:text-[#d4a843] active:text-[#d4a843] disabled:opacity-30 transition-colors p-2"
              >
                <ChevronLeft size={18} />
              </button>
...
function SectionHeader({ title, count }: { title: string; count?: number }) {
  return (
    <div className="flex items-center gap-2 sm:gap-3">
      ...
      <span className="text-[#d4a843] text-[9px] sm:text-[10px] font-extrabold tracking-[0.15em] sm:tracking-[0.18em] uppercase"
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="text-center py-8 sm:py-10 rounded-lg" style={{ background: "#060914", border: "1px solid #151f38" }}>
      <span className="text-[#2a3a54] text-xs sm:text-sm" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{text}</span>
    </div>
  );
}
...
function FilterButton({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="px-2 sm:px-3 py-1.5 rounded text-[9px] sm:text-[10px] tracking-wider uppercase transition-all whitespace-nowrap shrink-0"
      style={{
        fontFamily: "'JetBrains Mono', monospace",
